...
public:
    ...
    */
    MetaPath(string metaPathStr){
        vertex i = 0; // 1. i is use to select current num saved to vertex type or edge type;
...
public:
    ...
    static long double densityMap(vector<int> *a, vector<int> *b, long double p){
        ...
        for (vertex i = 0; i < dMapLen; i++) {

            vertex vLen = end - start;
...
public:
    ...
    op mnc_optimal_matrix_chain_order(vertex len, 
                                      BoolMatrix ** matrices, 
...
BoolMatrix *SpGEMM(BoolMatrix *mtxA, BoolMatrix *mtxB, vector<vertex> *vector_arr, bool *is_existed, vertex n){
    ...
    vertex tnum = omp_get_max_threads();
...
void SSP_dynamic(vector<HIN_nb> **HIN, BoolMatrix *&graph,
    ...
    vertex thread_num = omp_get_max_threads();
    ...
    dy_op->mnc_optimal_matrix_chain_order(p_l, matrices, bins, dims, &p_vts, n);
...
{   
    ...
    dy_op->free_mnc_optimal_matrix_chain_order(); 
    delete dy_op;
    gettimeofday(&end, NULL);
